...
*	@param[in]	len - length of private mode.
*	@return	 	none.
*/
static inline void rf_fix_payload_len_set(int len)
{
    write_reg8(0x404, read_reg8(0x404)|0x03); //select private mode header mode
...
/**
 * @brief      This function process the received packet in 1mbps private mode mode only for hanshow for the
 ...
 * @param[in]  len - the expected rx length of private mode mode, containing payload and 3byte crc
...
/**
 * @brief      This function process the tx packet in 1mbps private mode mode only for hanshow for the
 *             compatibility with third-party chips. The process includes data-whitening
               transformation and crc padding.
 * @param[in]  tx_buf - the rf tx buffer containing the tx packet(dma length+payload+3 byte crc)
 * @param[in]  len - the expected tx length of private mode mode, containing payload and 3byte crc
 * @return     none
 */
